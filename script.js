...
function drawCards(){
  ...
  picks.forEach((card, i) => {
    ...
    // ★ 2. 如果逆位，為 <img> 加 reversed
    front.innerHTML = `<img src="cards/back.png" ${isReversed ? 'class="reversed"' : ''}>`;

    const back = document.createElement('div');
    back.className = 'card-back';
    // ★ 3. 同樣在正面圖加 reversed
    back.innerHTML = `<img src="cards/${card}.png" ${isReversed ? 'class="reversed"' : ''}>`;
